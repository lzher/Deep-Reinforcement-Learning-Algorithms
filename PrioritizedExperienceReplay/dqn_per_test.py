from dqn_per import DQN, PrioritizedReplayMemory
import gym
import datetime
import os
...
save_steps = np.zeros((N_TIMES, N_EP))

dqn = DQN(s_dim, a_val, a_dim)
mem = PrioritizedReplayMemory(s_dim, a_dim, N_MEM)
    ...
    for ep in tqdm(range(N_EP)):
        ...
        while True:
            ...
            if done or (step >= N_MAX_STEP):
                ...
                log_fp.flush()
                # print(report)
                save_steps[t, ep] = step
                break

step_fn = LOG_PATH + '/steps'
sio.savemat(step_fn, {'steps': save_steps})

plt.plot(save_steps.mean(0))
